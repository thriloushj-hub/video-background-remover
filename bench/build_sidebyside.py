...
**Everything streams.**  The first version of this script read all three videos
into lists and was killed by the OOM reaper on the first full-length clip it
was pointed at: 774 frames of 1920x1080x3 is 4.8 GB for the source alone,
before v1 and v2.  Three capture handles advanced in lockstep cost three
frames of memory instead, which is why this runs on full clips at all.
...
def skip(cap, n, what):
    ...
    Deliberately not CAP_PROP_POS_FRAMES: seeking lands on the nearest
    keyframe on some encodes, which would silently shift the alignment by a
    few frames -- the exact class of error this script exists to avoid.
    """
    for i in range(n):
        if not cap.read()[0]:
            raise SystemExit(f"{what}: ran out of frames while skipping to {n}")
...
def make_bg(kind, h, w):
    ...
    bg = np.repeat(top[None, None, :] * (1 - t) + bot[None, None, :] * t,
                   w, axis=1)
...
def label(panel, text, sub=""):
    ...
    if sub:
        (tw, _), _ = cv2.getTextSize(sub, cv2.FONT_HERSHEY_SIMPLEX, 0.44, 1)
        cv2.putText(strip, sub, (w - tw - 12, 23), cv2.FONT_HERSHEY_SIMPLEX,
                    0.44, (150, 150, 150), 1, cv2.LINE_AA)
...
def main():
    ...
    cs, c1, c2 = opencap(a.source), opencap(a.v1), opencap(a.v2_alpha)
    lo = max(0, a.offset)
    skip(cs, lo, "source")
    skip(c2, lo, "v2 alpha")
    skip(c1, lo - a.offset, "v1")

    ok, s0 = cs.read()
    if not ok:
        raise SystemExit("no overlap between the three inputs")
    h0, w0 = s0.shape[:2]
    w = a.max_w
    h = int(round(h0 * w / w0))
    bg = make_bg(a.bg, h, w)
    fps = cs.get(cv2.CAP_PROP_FPS) or 24.0
    ...
    while True:
        ...
        if not (ok1 and ok2):
            break
        s = cv2.resize(frame_s, (w, h), interpolation=cv2.INTER_AREA)
        a1 = alpha_from_greenscreen(
            cv2.resize(f1, (w, h), interpolation=cv2.INTER_AREA), GREEN)
        ...
        panel = np.hstack([
            label(s, "source", f"from source frame {lo}" if n == 0 else ""),
            label(over(s, a1, bg), "v1  (alpha recovered from its green key)"),
            label(over(s, a2, bg), "v2  (real alpha, product pipeline)"),
        ])
        if vw is None:
            vw = cv2.VideoWriter(a.out, cv2.VideoWriter_fourcc(*"mp4v"), fps,
    ...
    if vw is not None:
        vw.release()
    print(f"{os.path.basename(a.out)}  {n} frames @ {fps:.3f}  "
          f"source {lo}-{lo + n - 1}  offset {a.offset}  bg {a.bg}")
