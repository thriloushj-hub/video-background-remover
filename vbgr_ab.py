"""3.2 -- motion-blur fix A/B on the ipman fast-motion window.
    ...
    !cd /content && unzip -o vbgr_package.zip && python vbgr_ab.py ipman all
    !cd /content && python vbgr_ab.py 1917 all
...
# v1 baselines on these exact windows, native resolution, scored with this code.
# "target" is the clip's own median subject coverage where one is known.
CLIPS = {
    "ipman": dict(
        file="/content/1b_ipman_fastwindow_3s.mp4",
        v1=dict(area_cv=0.124, edge_soft=0.3792, dropouts=0, mean_area=0.2874),
        note="source frames 318-389. Replacement window, verified single shot."),
    "1917": dict(
        file="/content/3_1917_3s_1080p.mp4",
        v1=dict(area_cv=0.0233, edge_soft=0.3659, dropouts=0, mean_area=0.3051),
        target=0.166,
        note="verified single shot. v1 holds an extra background soldier for "
             "the whole window, so its 0.3051 is roughly double the 0.166 the "
             "clip median says one soldier should be -- area_cv 0.0233 is blind "
             "to it because the error is steady. Watch mean_area."),
    "butter_old": dict(
        file="/content/2_butter_3s_1080p.mp4",
        v1=dict(area_cv=0.3720, edge_soft=0.3697, dropouts=0, mean_area=0.4124),
        target=0.28,
        note="WINDOW INVALID -- source frames 156-227, two hard cuts at source "
             "162 and 199 (window frames 6 and 43). Any number from it measures "
             "propagation across a cut. Kept only so the cut guard can refuse "
             "it. Use 'butter'."),
    "butter": dict(
        file="/content/2b_butter_cleanshot_3s.mp4",
        v1=dict(area_cv=0.6878, edge_soft=0.1062, dropouts=0, mean_area=0.1815),
        note="REPLACEMENT window, source frames 12-83, verified single shot "
             "(max hist dist 0.060 against a 0.20 cut threshold). Longest "
             "clean shot in the 382-frame source, and the highest-motion one "
             "(95th-pct flow 37.0 px/frame). v1 holds 4 subjects at frame 0 "
             "and scores area_cv 0.6878 -- very unstable -- with edge_soft "
             "0.1062, i.e. a hard cut-out edge. No clip-median target has been "
             "established for this window; the old 0.28 belongs to the old "
             "window and must not be reused."),
}

CLIP = WORK = FRAMES = None
V1_NATIVE = {}
...
def main():
    global CLIP, WORK, FRAMES, V1_NATIVE
    name = (sys.argv[1] if len(sys.argv) > 1 else "ipman").lower()
    if name not in CLIPS:
        sys.exit(f"usage: python vbgr_ab.py [{'|'.join(CLIPS)}] [single|off|on|all]")
    spec = CLIPS[name]
    CLIP = spec["file"]
    WORK = f"/content/ab_{name}"
    ...
    # ---- table ----------------------------------------------------------- #
    print("=" * 74)
    print(f"3.2  {name}, native, {len(frames)} frames")
    print("=" * 74)
    cols = ["v1"] + arms
    ...
    if "target" in spec:
        print(f"\nclip median target for mean_area is about {spec['target']}.")
        for c in arms:
            d = runs[c]["mean_area"] - spec["target"]
            print(f"  {c:8}{runs[c]['mean_area']:>9}  {d:+.4f} from target")

    with open(f"{WORK}/results.json", "w") as fh:
        json.dump({"clip": name, "note": spec["note"], "n_seed": n_seed,
